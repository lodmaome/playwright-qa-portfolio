...
export interface CheckoutFieldScenario {
  /** Unique identifier used in test names. */
  id: string;
  firstName: string;
  lastName: string;
  postalCode: string;
  /** When undefined the test expects *success* (overview page loaded). */
  expectedError?: string;
  rationale: string;
}

// Error cases
export const CHECKOUT_ERROR_SCENARIOS: readonly CheckoutFieldScenario[] = [
  // First name
  {
  ...
  },
  {
    ...
    rationale:
      "Boundary: whitespace-only first name — documents app behaviour.",
  },
  ...
  // All missing
  {
    ...
    postalCode: "",
    expectedError: Messages.CHECKOUT_INFORMATION_PAGE.MISSING_FIRST_NAME,
    rationale: "EC-4: all fields missing — UI reports the first error only.",
  },
] as const;

// Success cases (boundary values on valid inputs)
export const CHECKOUT_SUCCESS_SCENARIOS: readonly Omit<
  CheckoutFieldScenario,
  "expectedError"
>[] = [
  {
